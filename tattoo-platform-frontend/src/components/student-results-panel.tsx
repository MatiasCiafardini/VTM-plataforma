...
export function StudentResultsPanel({
  ...
  const totalRevenue = evolution.reduce((sum, period) => sum + (period.ingresosFacturacion ?? 0), 0);
  ...
  const bestPeriod = [...evolution].sort(
    (left, right) => (right.ingresosFacturacion ?? 0) - (left.ingresosFacturacion ?? 0),
  )[0];
  const revenueLabel = metricLabels?.revenue ?? 'Facturacion';
  ...
  const maxRevenue = Math.max(1, ...chartPeriods.map((period) => period.ingresosFacturacion ?? 0));
  ...
  const selectedExistingPeriod =
    evolution.find((period) => period.month === form.month && period.year === form.year) ?? null;
  const isExistingPeriodLocked =
    selectedExistingPeriod?.status === 'SUBMITTED' || selectedExistingPeriod?.status === 'CLOSED';
    ...
    if (field === 'month' || field === 'year') {
      setForm((current) => {
        ...
        const matchingPeriod =
          evolution.find((period) => period.month === nextMonth && period.year === nextYear) ??
          undefined;
  ...
  async function persistPeriod(submitAfterSave: boolean) {
    if (isExistingPeriodLocked) {
      setError('Ese mes ya fue enviado o cerrado, y no se puede editar desde alumno.');
      return;
    }
    if (submitAfterSave) {
    ...
    try {
      const createdPeriodResponse = await fetch('/api/student/metrics/periods', {
        method: 'POST',
      ...
      if (!createdPeriodResponse.ok) {
        ...
        const existingPeriods = (await existingPeriodResponse.json()) as MetricPeriodResponse[];

        if (!existingPeriodResponse.ok || existingPeriods.length === 0) {
          throw new Error(createdPeriodPayload.message ?? 'No pudimos crear el periodo.');
        }
      ...
      if (!saveValuesResponse.ok) {
        throw new Error(saveValuesPayload.message ?? 'No pudimos guardar las metricas.');
      }
      ...
      setError(
        submitError instanceof Error ? submitError.message : 'No pudimos guardar el mes.',
      );
    } finally {
      setIsSaving(false);
  ...
  return (
    <>
      <section className="student-results-panel">
        <header className="student-results-header">
          ...
          <div className="student-results-header-actions">
            <span className="student-results-currency">{localCurrency?.code ?? 'USD'}</span>
            {latestDraftPeriod ? (
              <button
            ...
            <button
              ...
              onClick={() => {
                ...
                const matchingPeriod =
                  evolution.find(
                    (period) => period.month === initial.month && period.year === initial.year,
                  ) ?? undefined;
                setForm(buildFormStateForPeriod(initial.month, initial.year, matchingPeriod));
                setError(null);
                setIsOpen(true);
              }}
            >
        ...
        <div className="student-results-summary-grid">
          ...
          <article className="student-results-summary-card">
            ...
            <strong>{revenueGrowth >= 0 ? '+' : ''}{revenueGrowth}%</strong>
            <p>Variacion del primer al ultimo mes</p>
          </article>
          <article className="student-results-summary-card">
            <span>Mejor mes</span>
            <strong>
              {bestPeriod ? formatMonthLabel(bestPeriod.month, bestPeriod.year) : 'Sin datos'}
            </strong>
              ...
              >
                <polyline
                  points={chartPeriods
                    .map((period, index) => {
                      const x =
                        chartPeriods.length === 1 ? 50 : (index / (chartPeriods.length - 1)) * 100;
                      const y =
                        100 - (((period.ingresosFacturacion ?? 0) / chartMax) * 78 + 8);
                      return `${x},${y}`;
                    })
                    .join(' ')}
                />
                  ...
                  return (
                    ...
                    >
                      <span className="student-results-chart-bar-wrap">
                        ...
                        />
                        {isActive ? <span className="student-results-chart-cursor" /> : null}
                      </span>
          ...
          <div className="student-results-growth-strip">
            {chartPeriods.slice(1).map((period) => {
              ...
              return (
                ...
                >
                  {positive ? '+' : ''}
                  {Math.round(delta.percentage)}% {formatShortMonth(period.month, period.year)}
                </span>
              );
            })}
          </div>
        ...
        <div className="student-results-history">
          {evolution
            ...
            .map((period) => {
              ...
              const totalOrigins = Math.max(1, newClients + recommendations + recurring);
                  ...
                  <div className="student-history-card-main">
                    <div className="student-history-card-top">
                      <div>
                        ...
                        <span className="student-history-status">{period.status}</span>
                      </div>
                    ...
                    <div className="student-history-metrics-grid">
                      ...
                      <div>
                        <span>Tatuajes</span>
                        <strong>{formatCompactNumber(period.cantidadTotalTatuajes)}</strong>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <strong>{formatCompactNumber(period.conversacionesANuevos)}</strong>
                      </div>
                      <div>
                        <span>{leadsLabel}</span>
                        <strong>{formatCompactNumber(period.consultasMensuales)}</strong>
                      </div>
                    </div>
      ...
      {isOpen ? (
        <div className="student-results-modal-backdrop" onClick={() => setIsOpen(false)}>
          <div
          ...
          >
            <div className="student-results-modal-head">
              <div>
                <h3>Cargar mes</h3>
                <p>Completá tu resumen mensual y se agregará al histórico de resultados.</p>
              </div>
              ...
              <article className="student-form-card">
                ...
                {selectedExistingPeriod ? (
                  <p className="student-form-card-copy">
                    Mes existente detectado: estado {selectedExistingPeriod.status}. Se cargaron
                    sus valores actuales en el formulario.
                  </p>
                ) : null}
